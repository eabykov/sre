...
| **MTTM** | Время устранения влияния отличается от MTTR тем, что, возможно, есть поломка, но пользователи или партнеры не имеют проблем с нашим сервисом из-за того что их переключили на резервную систему
...
### **Универсальные рекомендации по улучшению ключевых метрик**  

| **Метрика** | **Самая действенная рекомендация** | **Пример применения** |
|-|-|-|
| **SLI/SLO** | Автоматическое масштабирование ресурсов при приближении к пороговым значениям. | При заполнении диска БД >70% автоматически увеличивать его объем (например, AWS Auto Scaling + CloudWatch). |
| **Error Budget** | Приоритезировать задачи по стабильности при исчерпании бюджета. | Если Error Budget исчерпан на 80%, блокировать релизы новых функций до восстановления бюджета. |
| **MTTD** | Внедрить прогнозирующие алерты на основе ML. | Использовать алгоритмы для предупреждения о риске заполнения диска за 1 час до критического уровня. |
| **MTTR/MTTM** | Автоматизировать процедуры восстановления через Infrastructure as Code (IaC). | Написать Terraform-скрипт для очистки диска и переключения трафика на резервную БД. |
| **RTO/RPO** | Регулярно тестировать аварийное переключение и синхронизацию данных. | Проводить ежемесячные тесты переключения на резервную БД с проверкой RPO ≤ 5 мин. |
| **STAMP** | Внедрить автоматические ограничители для предотвращения "состояний опасности". | Настроить автоматическое отключение нежизнеспособных компонентов при достижении 90% нагрузки. |
| **Postmortem** | Автоматизировать сбор данных для анализа и внедрения исправлений. | Использовать шаблоны в Jira для автоматического создания задач по результатам Postmortem. |
