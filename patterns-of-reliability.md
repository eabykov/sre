...
#### 5. **Fail Fast (Быстрый отказ)**  
**Суть:** Система немедленно отклоняет запрос, который заведомо не может быть обработан, вместо того чтобы держать его в очереди и истощать ресурсы. Позволяет быстро вернуть управление клиенту и сохранить стабильность для остальных запросов.  
**Пример:** Веб-сервер ограничивает одновременные подключения до 1000. Запросы сверх лимита немедленно отклоняются с кодом 503 «Сервер перегружен», сохраняя работоспособность для текущих пользователей.
